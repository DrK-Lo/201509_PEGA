...
### Get list of superoutliers pine
  ...
  PE$pine_score1_ALL_corrected <- PE$pine_score2_ALL_corrected <- NA
  PE$pine_score1_ALL_raw <- PE$pine_score2_ALL_raw <- NA
  PE$pine_score_super_t <- PE$pine_score_super_t_raw <- PE$pine_score_super_g <- PE$pine_score_super_g_raw <- NA
...
### Get list of superoutliers spruce
  ...
  PE$spruce_score1_ALL_corrected <- PE$spruce_score2_ALL_corrected <- NA
  PE$spruce_score1_ALL_raw <- PE$spruce_score2_ALL_raw <- NA
  PE$spruce_score_super_t <- PE$spruce_score_super_t_raw <- PE$spruce_score_super_g <- PE$spruce_score_super_g_raw <- NA
```
...
### Get relevant columns from large file (pine)
  ...
  GWAS_pheno_name_raw <- GWAS_pheno_name  # next two lines fix a problem specific to pine df
  if(GWAS_pheno_name=="root_wt__shoot_wt_p"){GWAS_pheno_name_raw <- "root_wt_shoot_wt_p"}
  if(GWAS_pheno_name=="root_wt__shoot_wt_p_1"){GWAS_pheno_name_raw <- "root_wt_shoot_wt_p_1"} 
  GWAS_effect_col_pine_raw <- which(results_pine_head==paste(GWAS_pheno_name_raw, "_raw_rho", sep=""))
...
plotter <- function(GWAS_effect, GEA_rho_polarized, main, xlim, ylim, xlab, ylab){
  ...
  png(paste("analysis/figure_byPheno/PEGA_" ,PE_pheno_name,"_",i, "_", PE_envi_name,".png",sep=""), width=10, height=10, units="in", res=300)
  par(mfrow=c(2,2), mar=c(4,4,4,1), oma=c(0,0,3,3), cex.main=0.8)

  a.1 <- plotter(GWAS_effect_pine_standardized_corrected, GEA_rho_pine_polarized_corrected, 
          main="All SNPs Standardized", xlim=20, ylim=1,
          xlab = "GWAS corrected PINE", ylab="GEA corrected polarized PINE")
      PE$pine_score1_ALL_corrected <- a.1[1]
      PE$pine_score2_ALL_corrected <- a.1[2]

  a.2 <- plotter(GWAS_effect_spruce_standardized_corrected, GEA_rho_spruce_polarized_corrected, 
          main="All SNPs Standardized", xlim=20, ylim=1,
          xlab = "GWAS corrected SPRUCE", ylab="GEA corrected polarized SPRUCE")
      PE$spruce_score1_ALL_corrected <- a.2[1]
      PE$spruce_score2_ALL_corrected <- a.2[2]

  a.3 <- plotter(GWAS_effect_pine_raw, GEA_rho_pine_polarized_raw, 
          main="All SNPs RAW", xlim=1, ylim=1,
          xlab = "GWAS RAW PINE", ylab="GEA RAW polarized PINE")
        PE$pine_score1_ALL_raw <- a.3[1]
        PE$pine_score2_ALL_raw <- a.3[2]      

  a.4 <- plotter(GWAS_effect_spruce_raw, GEA_rho_spruce_polarized_raw, 
          main="All SNPs RAW", xlim=1, ylim=1,
          xlab = "GWAS RAW SPRUCE", ylab="GEA RAW polarized SPRUCE")
        PE$spruce_score1_ALL_raw <- a.4[1]
        PE$spruce_score2_ALL_raw <- a.4[2]
