...
```{r}
plot_2D.b <- function(x,y, xlab, ylab, xlim=c(-0.3,0.3), ylim=c(-0.3,0.3), nbin, x_sub_orange=NULL, y_sub_orange=NULL, x_sub_green=NULL, y_sub_green=NULL, x_sub_blue=NULL,  y_sub_blue=NULL,x_sub_golden=NULL,y_sub_golden=NULL, PE=1){
  ...
  if(length(ylim)==0){
    ...
    plot_mycov_sub <- function(x_sub, y_sub, pch, color){
      if(length(x_sub)>0){
      points(x_sub, y_sub, pch=24, cex=1, col="darkorange", bg="orange")
      plot_mycov(data.frame(x_sub_orange, y_sub_orange), col="darkorange", lty=2)
    }
    }
      ...
      for(type in c("Bayenv", "Raw")){
        ...
        ### because matching SNPs are based on maf and would be the same
          ...
          plot_2D.b(x1,y1, 
             ...
             ylab= "" , ylim=c(-lim,lim),
             nbin=100, 
             x_sub=x1[Group2_logical], 
             y_sub=y1[Group2_logical],
             PE=round(pecorr,2),
