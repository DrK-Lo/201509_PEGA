...
#labs = results_pine3$gtcontig[results_pine3$Group1_logical]
...
labs <- results_pine3$gtcontig_pos[results_pine3$Group1_logical]
...
#table(labs)
snp_order$labels <- labs4
str(snp_order)

### Get distances among SNPs 
codist <- cophenetic(snp_order)
hist(codist)
str(codist)

### Try plotting relationships with codist > 1 or >1.25
...
### Create undirected network graph
  graph_cors2 <- d4 %>%
    filter(abs(value) < 1.0) %>%
    graph_from_data_frame(directed = FALSE)
  
  # check node names
  V(graph_cors2)$name
  # line up names
  myo <- match(V(graph_cors2)$name, snp_order$labels)
  myocol <- results_pine3$clustercol[results_pine3$Group1_logical][myo]
  z <- cbind(myocol, V(graph_cors2)$name, snp_order$labels[myo])
  table(z[,1]) #check against colors, think about right
  
### 
  pdf(file = "results/Network.pdf", width=20, height=20)
  ggraph(graph_cors2) +
