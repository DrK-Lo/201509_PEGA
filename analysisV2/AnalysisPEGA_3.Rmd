...
results_pine3$clustercol[results_pine3$cluster==1] = "orange" #Aridity
results_pine3$clustercol[results_pine3$cluster==2] = "yellow" #Geography 
results_pine3$clustercol[results_pine3$cluster==3] = "blue" #Freezing
results_pine3$clustercol[results_pine3$cluster==4] = "green" #Multi
head(results_pine3$clustercol[results_pine3$Group1_logical])
...
snp_distmat_multi <- snp_distmat[shortclust ==4, shortclust ==4]
dim(snp_distmat_multi)
...
head(d4_multi)
d4_multi$value <- log10(d4_multi$value)
hist(d4_multi$value)

### Create undirected network graph for multi group
  graph_cors_multi <- d4_multi %>%
    #filter(value < 0.3) %>% 
    filter(value < -0.4) %>% 
    graph_from_data_frame(directed = FALSE)
  # check node names
  V(graph_cors_multi)$name
  # line up names
  (myo <- match(V(graph_cors_multi)$name, results_pine3$gtcontig_pos))
  (myocol <- results_pine3$clustercol[myo])
  V(graph_cors_multi)$name <- gsub("_", "\n",V(graph_cors_multi)$name )
  z <- cbind(myocol, V(graph_cors_multi)$name, results_pine3$gtcontig_pos[myo])
  table(z[,1]) #check against colors
  
### 
  pdf(file = "../results/Network_Multi.pdf", width=20, height=20)
  ggraph(graph_cors_multi) +
  geom_edge_link(aes(edge_alpha = 0.2, 
                     edge_width = 5e-05, color=value)) +
  guides(edge_alpha = "none", edge_width = "none") +
  scale_edge_colour_gradientn(limits = c(min(d4_multi$value, na.rm=TRUE),max(d4_multi$value, na.rm=TRUE)), 
                              colors = c("firebrick2", "dodgerblue2")) +
  geom_node_point(color = myocol, size = 30) +
...
```{r}
shortclust <- results_pine3$cluster[results_pine3$Group1_logical]
snp_distmat_multi <- snp_distmat[shortclust ==1, shortclust ==1]
dim(snp_distmat_multi)
head(snp_distmat_multi[1:5, 1:5])
hist(snp_distmat_multi)
  # This histogram is bimodal with the larger values > 0.4
...
### Create undirected network graph for multi group
  graph_cors_multi <- d4_multi %>%
    ...
    filter(value < -0.4) %>% 
    graph_from_data_frame(directed = FALSE)
  write.graph(graph_cors_multi, file = "../results_AridityNetwork.gml", format="gml")
  # check node names
  V(graph_cors_multi)$name
  # line up names
  (myo <- match(V(graph_cors_multi)$name, results_pine3$gtcontig_pos))
...
### 
  pdf(file = "../results/Network_Aridity.pdf", width=20, height=20)
  ggraph(graph_cors_multi) +
  geom_edge_link(aes(edge_alpha = 0.2, 
                     edge_width = 5e-05, color=value)) +
  guides(edge_alpha = "none", edge_width = "none") +
