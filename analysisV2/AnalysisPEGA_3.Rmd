...
# HORIZONTAL
contig.group.df<- contig.group.df[order(contig.group.df$Multi, contig.group.df$Aridity, contig.group.df$Freezing,   contig.group.df$Geography, decreasing=TRUE),]
  toplot <- t(as.matrix(contig.group.df[,c(4,1,3,2)]))
  head(toplot)
  dim(toplot)
  dim(contigdat)
  m <- match(colnames(toplot), contigdat$gtcontig)
  colnames(toplot)[1] 
  contigdat$gtcontig[84]
  colnames(toplot) <- contigdat$contigID[m]
  head(toplot)
  col_toplot <- matrix(rep(contigdat$col[m], each=4), nrow=4)
  head(col_toplot)
  
pdf("../results/PineVenn_horiz.pdf", width=20, height=6)
  par(mar=c(4,6, 1,1))
  barplot(toplot, beside = FALSE, horiz = FALSE, las=2, col=c("lightgreen", "orange", "lightblue",  "yellow"), cex.names=1, cex.axis = 2,  border = c("darkgreen", "brown", "blue",  "brown"), cex=2,  ylab="Number of SNPs", xlab= "Contig ID", ylim=c(0,50))
  
  #barplot(toplot, beside = FALSE, horiz = FALSE, las=2, col=col_toplot, cex.names=1, cex.axis = 2,  border = c("darkgreen", "orange", "blue",  "yellow"), cex=2)
  
  legend(80, 50, c("Multi", "Aridity", "Freezing",  "Geography"), bty="n", fill=c("lightgreen", "orange", "lightblue",  "yellow"), border = c("darkgreen", "brown", "blue",  "brown"), cex=1.5)
  #text(40, 5, "Multi", col="darkgreen", cex=1.5, adj=0)
  #text(15, 15, "Freezing", col="blue", cex=1.5, adj=0)
  #text(15, 45, "Aridity", col="darkorange", cex=1.5, adj=0)
  #text(10, 90, "Geography", col="brown", cex=1.5, adj=0)
...
MakeDistNetwork <- function(clustnum, filename, disthreshold, colordf, full_name=FALSE){
...
#### Multi Dist Network ####
mm <- MakeDistNetwork(4, "Multi", 0.1, colordf=contigdat) 
  head(mm)
...
mm <- MakeDistNetwork(1, "Aridity", 0.1, colordf=contigdat) 
  head(mm)
  ### Assign subcluster membership and number edges
  mind <- match(mm$gtcontig_pos, results_pine3$gtcontig_pos)
  results_pine3$subcluster[mind] <- mm$subcluster
...
mm <- MakeDistNetwork(2, "Geography", 0.1, colordf=contigdat) 
  head(mm)
  ### Assign subcluster membership and number edges
  mind <- match(mm$gtcontig_pos, results_pine3$gtcontig_pos)
  results_pine3$subcluster[mind] <- mm$subcluster
...
mm <- MakeDistNetwork(3, "Freezing", 0.1, colordf=contigdat)
  head(mm)
  ### Assign subcluster membership and number edges
  mind <- match(mm$gtcontig_pos, results_pine3$gtcontig_pos)
  results_pine3$subcluster[mind] <- mm$subcluster
  ...
  table(results_pine3$cluster, results_pine3$pine_super_convergent)
    # super convergent SNPs found in elevation freezing group
### 
MakeDistNetwork(1, "Aridity", 0.05, colordf=contigdat, full_name = TRUE) 
MakeDistNetwork(2, "Geography", 0.1, colordf=contigdat, full_name = TRUE) 
MakeDistNetwork(3, "Freezing", 0.05, colordf=contigdat, full_name = TRUE) 
MakeDistNetwork(4, "Multi", 0.1, colordf=contigdat, full_name = TRUE) 
```
...
# Cluster color for outline
  ...
  results_pine3$clustercol[results_pine3$cluster==1] = "brown"
...
# Contig color for points
  mm <- match(results_pine3$gtcontig[results_pine3$Group1_logical], contigdat$gtcontig)
  mm
  results_pine3$contigcol[results_pine3$Group1_logical] <- contigdat$col[mm]

pdf(file = "../results/GalaxyNew_inMATvMAP2.pdf", width=12, height=12)
...
galaxyplot(x=x, y=y, 
       xlab= "Spearman's rho between allele frequency and and Mean Annual Temperature", 
       ylab= "Spearman's rho between allele frequency and Elevation" , 
       nbin=100, xlim=c(-0.8,0.8), ylim=c(-0.8,0.8),PE=-0.48, PElab=c("MAT", "ELEVATION"))

galaxyplot(x=x, y=y, 
       xlab= "Spearman's rho betweenallele frequency and Mean Annual Temperature", 
       ylab= "Spearman's rho between allele frequency and Elevation" , 
       nbin=100, xlim=c(-0.8,0.8), ylim=c(-0.8,0.8),
       x_sub=x[results_pine3$Group1_logical], 
       y_sub=y[results_pine3$Group1_logical],
...
galaxyplot(x=x, y=y, 
       ...
       nbin=100, xlim=c(-0.8,0.8), ylim=c(-0.8,0.8),PE=-0.48, PElab=c("MAT", "LAT"))
