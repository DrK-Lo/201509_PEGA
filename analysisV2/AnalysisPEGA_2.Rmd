...
identical(paste(results_pine3$gcontig, results_pine3$pos_gcontig), paste(results_pine3$CHROM, results_pine3$POS))
...
results_pine3$Ances_flip <- as.character(results_pine3$Ances_flip)
...
for (i in 1:nrow(results_pine3)){
  ...
  bob <- which(c(results_pine3$minor_allele[i], results_pine3$major_allele[i])==results_pine3$Ances_flip[i])
    # which allele equals the ancestral allele
  boborder <- order(c(results_pine3$minor_allele[i], results_pine3$major_allele[i]))
    # order the alleles in pine
  
  # the allele earlier in the alphabet is coded as a 1, later is a 0
  
  if(length(bob)>0){
    if (boborder[bob]==1){
      # the ancestral SNP is the allele that comes earlier in the alphabet
      # the earlier allele was coded as 1, but now want coded as 0
      flipnum[i] <- -1
    }
     # the ancestral SNP is NOT the allele that comes earlier in the alphabet
     # the earlier allele is the derived allele and should be already coded as 1
    if(boborder[bob]==2){
      flipnum[i] <- 1
    }
  }#end if length(bob) 
...
```{r, label="load data"}
save.image("../data/large_files/Pine_Alpha_AveRho_WithSuperLogical_andAncestralFlip.RData")
```
